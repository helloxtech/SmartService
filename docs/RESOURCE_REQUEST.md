...
- P0 credential groups present: Supabase public runtime config, fictional demo identities, OpenAI, Cloudflare Worker/Queues/R2 buckets, and hosted DEV public smoke are ready; local hosted Supabase admin seed/count still needs the online service-role key or connector SQL; Browser Run token, R2 S3 signer credentials, and Turnstile site/secret keys are missing, so G1 live integration remains blocked
