...
| Supabase server configuration | READY LOCAL | Secret server key, database URL, and database password for the dedicated project | `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_DATABASE_URL`, and `SUPABASE_DB_PASSWORD` are stored only in ignored `.env.local`; Worker secret placement remains pending |
