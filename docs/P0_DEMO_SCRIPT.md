...
Run the complete Smart Service text-service loop in 5–7 minutes without production data or paid provider calls. The fixed local identities and passwords remain in `.env.local`; never display them during the demo.
...
- Team profile: `/app/dashboard`, signed in as the fictional Smart Service Admin

Keep the fictional Smart Service Agent available in a third private profile. The HarborWorks Admin is the backup organization for the isolation proof.

## Main 5–7 minute flow

### 0:00–0:45 — Approved knowledge

1. Open **Knowledge** as the Smart Service Admin.
...
Expected: Smart Service refuses to invent an answer, creates one grouped open gap, and requests human support without citations.

### 2:15–3:15 — Human takeover and close

1. Open **Inbox** as the Smart Service Agent.
...
1. Return to **Dashboard** as the Smart Service Admin.
...
3. Confirm the Smart Service conversation, metrics, and gap are absent.
...
- If the main Smart Service profile cannot sign in, use the HarborWorks profile only to demonstrate isolation, then stop. It is not a substitute for the primary P0 flow.
