...
### 4.3A 2026-08-07 演示前批准范围扩展

- R10 人工客服回复辅助：只在转人工后按最新客户消息异步生成企业通用、有引用、受红线保护的建议话术。
- 建议话术必须由当前人工客服选择、编辑和发送；系统不得自动发给客户。
- 每个客户消息只保留一个当前建议；新消息、人工回复、结束会话或知识来源失效时，旧建议必须失效并保留审计。
- 不增加新的模型、知识管线、Queue、付费方案或行业特定逻辑；复用 P0 RAG、引用、红线、租户和现有 Queue 边界。
