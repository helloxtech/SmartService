...
| Local secret file | BLOCKING-NOW | READY | Ignored mode-`0600` `.env.local` contains local Supabase/demo/signing values plus locally verified OpenAI, LiveKit, Deepgram, and ElevenLabs credentials; the Browser Run token is held only as a hosted Worker secret, while P0 R2 signer and Turnstile values are still missing; values were not displayed |
...
| Supabase server configuration | READY DEV / LOCAL GAP | Secret server key, database URL, and database password for the dedicated project | `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are set as Cloudflare Worker secrets and hosted public smoke passes. The ignored local `.env.local` service-role value is not valid for hosted project `ibuvpregltbvxsxhivrg`, so local hosted seed/count tooling needs the online service-role key stored locally or working connector SQL. |
...
| Browser Run token | READY | One account-scoped least-privilege token with Browser Run write access, stored only as `CLOUDFLARE_BROWSER_RUN_API_TOKEN` in Worker secrets and expiring August 2, 2027 | Live bounded crawl completed without printing or committing the token; do not replace it with a broad Wrangler/OAuth credential |
...
| Cloudflare Browser Run `/crawl` access | READY | Current account entitlement supports the bounded hosted-development crawl | Live same-origin crawl completed at the 10-page/depth-2 product bound and produced 65 embedded chunks |
...
| G1 preview/runtime location | READY DEV | Default is one Cloudflare Worker with Static Assets plus Hono API to avoid cross-origin drift | HTTPS, hosted Admin, live chat/auxiliary/embedding, live Browser Run crawl, cited answers, contextual confirmation, and safe handoff passed; full G1 still requires live Turnstile and the R2-signed upload path |
...
| Demo ingestion corpus | READY | Deterministic fixtures remain committed; the current hosted tenant also contains the user-supplied website, DOCX, and approved manual answer | All three supplied sources are Ready with 10 website documents and 79 total chunks; three obsolete seeded demo sources are disabled and reversible |
| Bounded demo website | READY | The supplied public music-school site is constrained to same-origin, at most 10 pages, and depth 2; advanced redirect/DNS fixtures remain part of pre-G1 hardening | Live Browser Run crawl passed and produced 10 documents and 65 OpenAI-embedded chunks |
...
| LiveKit CLI | READY | Homebrew `lk` 2.18.2; project can be addressed non-interactively | Cloud device authorization completed against the signed-in SmartService Development project without exposing credentials |
...
| LiveKit Agent deployment | READY DEV | Agent `CA_bifEfej5s7Di` is deployed from the repository Dockerfile; active version `MhjBXbqQ5WDa` maps to commit `01b3d05` | Running/Available replica, named dispatch, shared-API configuration, Ready-before-microphone startup, final-turn orchestration, and mobile playback recovery passed automated checks; final real-device spoken-turn evidence remains |
| LiveKit budget/minute boundary | READY | Approved development Agent/WebRTC minutes use the visible free allowance; no paid plan was enabled | Bounded rooms only; stop test participants after each run and recheck usage before G2 |
...
| Final demo browser/device | VERIFY | A real mobile browser reached Listening and displayed speech transcription; the repaired answer/TTS boundary and explicit audio-unlock control are deployed | User speaks one Chinese and one English turn, taps the audio-enable control if shown, and verifies final transcript, grounded answer, and browser playback |
...
| HTTPS voice URL | READY DEV | `https://smartservice.ca/voice` serves Worker version `7af0357c-8e39-42a0-964a-039b5a503c8a` at 100% with `VOICE_PROVIDER_MODE=live` in the development environment | `/health`, `/voice`, `/chat`, live mobile playback-recovery asset, token, room, named Agent dispatch, and Ready passed; final real-device grounded-answer/TTS playback check remains |
...
| Cloudflare Browser Run | Live adapter and bounded hosted crawl are verified; deterministic fixture provider remains for routine tests | Re-verify before G1 only if the token, account, or crawl contract changes |
...
- P0 credential groups present: Supabase public runtime config, fictional demo identities, OpenAI, Cloudflare Worker/Queues/R2 buckets, Browser Run, and hosted live grounded-text acceptance are ready; local hosted Supabase admin seed/count still needs the online service-role key or connector SQL; R2 S3 signer credentials and Turnstile site/secret keys are missing, so full G1 remains blocked
- P1 credential groups present or deferred: LiveKit API credentials, Deepgram, and ElevenLabs are verified; LiveKit CLI cloud authorization, Cloud Agent deployment, public DEV live token/room/dispatch, Ready gating, and preview interim STT passed. Final device spoken-answer/TTS/interruption/latency evidence and ElevenLabs commercial-use confirmation remain missing, so G2 remains blocked
