...
**Last updated:** August 6, 2026 PDT
...
**Current phase:** The reported hosted knowledge and unintended-handoff failures are repaired and live-accepted; the remaining P0 work is the production-grade signed upload and Turnstile boundary
**Active step:** Hold at the remaining G1 resource boundary while the locally validated tenant customer-service ownership candidate awaits review and deployment authorization
**Overall state:** Days 1–10 remain green locally. Hosted DEV now uses Cloudflare-hosted GLM 4.7 Flash as the primary grounded-answer model; the validated `gpt-5-mini` fallback implementation remains available but is temporarily disabled with `CHAT_FALLBACK_PROVIDER=none` so every successful test answer is attributable to GLM. Evidence-aware auxiliary review, live OpenAI embeddings, and live Cloudflare Browser Run crawling remain unchanged; neither auxiliary review nor embeddings can provide the customer answer. The supplied music website is Ready with 10 documents and 65 chunks, the supplied DOCX is Ready with 13 chunks, and the approved manual answer is Ready with one chunk. Multi-part Chinese questions now use focused bilingual retrieval and field-by-field answers, Guqin cannot be substituted with Guzheng evidence, accurate missing-field language survives output supervision, exact crawled page URLs appear in citations, and one same-GLM retry absorbs transient schema/citation failures without calling the disabled GPT answer fallback. Live acceptance proved natural Guqin limitations with no handoff and cited 2001/founder/address answers that explicitly leave the current principal and home/online mode unconfirmed. Unsupported questions remain `active_ai`, create a canonically merged knowledge gap, and never claim or initiate human transfer; explicit customer requests and safety/guardrail escalation still hand off. Public chat merges polling and send completion by authoritative message ID, preventing duplicate answers, and lets a returning customer leave any restored conversation through a confirmed New conversation action without deleting the retained support history. Three obsolete seeded demo sources are disabled and no longer retrievable. Full P0 G1 is not claimed because the current development upload adapter still needs bucket-scoped R2 signer credentials and the public endpoint still needs hostname-bound Turnstile keys. P1 Agent deployment, final device evidence, and ElevenLabs commercial-use confirmation also remain pending.

## Recent hosted UAT fixes

- August 6, 2026 PDT: Updated the local `codex/admissions-voice-policy` candidate; it is not deployed. Text and voice customer surfaces now identify the experience as the current company's online customer service rather than an AI, an external researcher, or a school-specific admissions team. The shared RAG prompt and response normalizer state confirmed company facts directly, remove “根据我查到的资料” / “I checked” framing, and require a support-specialist option for any unconfirmed detail. The first clarification now offers a visible specialist CTA but does not transfer the conversation until the customer chooses it; the composer clears and becomes non-editable while an answer is pending. Guardrail and actual-handoff copy use the same tenant-neutral wording, including a safety-preserving automatic escalation path. Local checks passed assistant-core (28 tests), web unit/browser (24 tests), the targeted responsive Playwright flow (1 test), P0 evaluation (4 tests), guardrail evaluation (1 test), formatting, zero-warning lint, all workspace typechecks, production builds, and the Worker dry run. The full API Worker suite was blocked before test execution because this isolated worktree has no `CLOUDFLARE_API_TOKEN` for the remote test pool; no workaround, provider call, or deployment was attempted.
- August 3, 2026 PDT: Released the humanized multi-intent answer path through commits `a4aa700`, `9514682`, `b808833`, `48fc94d`, and `20b0c56`; functional Worker version `a1e1cec8-9a30-47f0-b4bb-20604fd9b36a` passed the final exact-question smoke. Retrieval now decomposes the four-part school question, appends bounded English hints to Chinese school intents, interleaves per-part results at an audited `0.30` cross-language threshold, and requires exact Guqin/Guzheng evidence. The assistant states that Guqin availability/fees were not found, offers optional human help without initiating it, answers the 2001 founding year and Richmond address with the exact About-page citation, and explicitly says the current principal and at-home/online mode were not found. The output supervisor accepts supported partial answers, blocked copy is language-normalized, customer text avoids internal evidence jargon, and a same-GLM retry handles one transient provider/schema/citation failure without calling OpenAI for the answer. Two consecutive compound turns succeeded through `cloudflare-workers-ai` / `@cf/zai-org/glm-4.7-flash`, remained `active_ai`, and had no handoff; the final Guqin three-turn sequence returned three natural clarifications, zero citations, and zero handoffs. The final complete gate passed 155 tests, both fixed evaluations, formatting, zero-warning lint, every workspace typecheck, production builds, and the Worker dry run.
- August 3, 2026 PDT: Temporarily disabled GPT answer fallback for primary-model testing in `d0a331f`. Cloudflare deployment `513fd129-80cd-4c9d-bfd8-bdd9135d0379` promoted Worker version `0dc78281-55fe-48d5-ac70-26cd05eb6da6` to 100% with `CHAT_PRIMARY_PROVIDER=workers-ai` and `CHAT_FALLBACK_PROVIDER=none`. A fresh English course turn returned five citations with no handoff; the Worker emitted only a successful `@cf/zai-org/glm-4.7-flash` event and the matching `ai_runs` record persisted `cloudflare-workers-ai`, 3,889 input tokens, 262 output tokens, and `succeeded`. Regressions prove that primary-only success never calls OpenAI and a Workers AI provider failure fails closed without an OpenAI request. The complete local gate passed 145 tests, both fixed evaluations, formatting, zero-warning lint, strict typechecks, production builds, and the Worker dry run. Re-enable the already-tested fallback by setting `CHAT_FALLBACK_PROVIDER=openai` and redeploying.
- August 3, 2026 PDT: Released the cost-optimized grounded-answer path in `dcab1c9`. Cloudflare deployment `2b211482-4e93-4577-a99e-a8bbbedd5a6c` promoted Worker version `26d502fe-d1fb-49d9-ac68-89b30123f113` to 100% with an `AI` binding, `@cf/zai-org/glm-4.7-flash` as the non-thinking primary, and `gpt-5-mini` as a one-attempt reliability fallback. The primary answer must still pass the existing schema, exact-retrieval citation, deterministic guardrail, and auxiliary-supervisor boundaries; provider-side content logging is disabled and the actual successful provider/model/token usage persists in `ai_runs`. Live GLM proof answered the English course question with five citations and the Chinese Guzheng question with three citations; the hosted suite passed 3/3 cited answers and 1/1 AI-active clarification, while the approved manual answer plus `are you sure?` retained one citation and never handed off. Provider-error and invalid-citation regressions prove fallback behavior. The complete local gate passed 143 tests, both fixed evaluations, formatting, zero-warning lint, strict typechecks, production builds, and the Worker dry run.
- August 3, 2026 PDT: Released the public-chat session escape in `e2fc06f`. A visible English/Chinese New conversation action above the composer now lets customers leave any restored state, including a pending human handoff, after confirmation. The action clears only that browser tab's scoped session, preserves the prior server conversation for Agent support/audit, requires fresh verification, and creates a separate server conversation on the next customer message. Worker version `b5e39db6-ca8b-486c-b894-f81fd9a5b1e9` serves the new localized UI and corrected no-auto-transfer notice; `/health` and `/chat` returned `200`, and the live asset checks passed. The regression covers cancel, confirmed reset, fresh conversation creation, retained old history, and no DELETE request. The full local suite passed 140 tests, formatting, zero-warning lint, strict typechecks, production builds, and the Worker dry run. Direct rendered live-browser inspection remained unavailable because the browser's administrator policy denied automated access to `smartservice.ca`; no policy bypass was attempted.
- August 3, 2026 PDT: Reproduced the customer's unexpected `等待人工客服接入` state and confirmed that missing knowledge was hard-coded as `decision=handoff`, even without customer intent. Released customer-controlled handoff policy in `cea4630`, canonical gap metadata/trigger handling in `b692cd1`, and deterministic model-question normalization in `8c6c684`; hosted migrations `20260803192827` and `20260803194616` are present remotely. Final functional Worker version `c69ea32d-60c6-457f-82b7-8a5a1c2183ca` passed 3/3 cited music-school answers, including `古筝课程要上多久？` as 40 hours with two citations, plus 1/1 unsupported clarification. Direct hosted-state verification proved `active_ai`, `decision=clarify`, zero handoffs, and one canonically normalized gap. The final local suite passed 139 tests and 20/20 database assertions. All 20 anonymous smoke conversations, their AI runs and audit rows, and four synthetic gaps created during acceptance were removed after verification.
...
| `pnpm eval:p0` | Passed: 4 tests, including deterministic 12/12 cited in-scope and 8/8 missing-knowledge AI-active clarification behavior; live GLM quality is separately evidenced by hosted UAT |
