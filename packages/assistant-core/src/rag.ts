...
function buildDeterministicFixtureAnswer(input: RagGenerationInput): RagAnswer
{
    ...
    )
    {
        result = createEvidenceAnswer(
            ...
            "定制产品通常 not returnable；只有 NovaFlow 确认存在 manufacturing defect 时例外。",
            "Custom-configured products are not returnable unless NovaFlow confirms a manufacturing defect.",
        );
    }
