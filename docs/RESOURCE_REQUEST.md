...
| LiveKit Agent deployment | READY DEV | Agent `CA_bifEfej5s7Di` is deployed from the repository Dockerfile; active version `hDn8Jig7ijt8` maps to commit `26cf964` | Running/Available replica, named dispatch, shared-API configuration, and Ready-before-microphone startup passed; final spoken-turn evidence remains |
...
| Final demo browser/device | VERIFY | Current Codex in-app browser and macOS microphone reached Listening on the preview origin; the public domain is waiting on its one-time microphone prompt | User accepts the per-origin prompt, speaks one Chinese and one English turn, and verifies transcript plus browser playback |
...
| HTTPS voice URL | READY DEV | `https://smartservice.ca/voice` serves Worker version `59da5945-fc6a-4f03-a66d-60628384dc80` at 100% with `VOICE_PROVIDER_MODE=live` in the development environment | `/health`, `/voice`, `/chat`, token, room, named Agent dispatch, and Ready passed; final microphone/TTS playback check remains |
