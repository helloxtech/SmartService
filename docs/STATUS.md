...
**Active step:** Hold at the remaining G1 resource boundary while the locally validated admissions-owner voice candidate awaits review and deployment authorization
...
- August 6, 2026 PDT: Prepared the local `codex/admissions-voice-policy` candidate; it is not deployed. Text and voice customer surfaces now identify the experience as the school's admissions team rather than an AI or external researcher. The shared RAG prompt and response normalizer state confirmed facts directly, remove “根据我查到的资料” / “I checked” framing, and require an admissions-manager option for any unconfirmed detail. The first clarification now offers a visible manager CTA but does not transfer the conversation until the customer chooses it; the composer clears and becomes non-editable while an answer is pending. Guardrail and actual-handoff copy use the same admissions-manager wording, including a safety-preserving automatic escalation path. Local checks passed assistant-core (28 tests), web unit/browser (24 tests), the targeted responsive Playwright flow (1 test), P0 evaluation (4 tests), guardrail evaluation (1 test), formatting, zero-warning lint, all workspace typechecks, production builds, and the Worker dry run. The full API Worker suite was blocked before test execution because this isolated worktree has no `CLOUDFLARE_API_TOKEN` for the remote test pool; no workaround, provider call, or deployment was attempted.
