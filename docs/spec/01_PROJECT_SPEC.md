...
当出现以下任一情况时，返回 `handoff`：

- 没有检索结果达到最低阈值。
- 结果相关，但没有足够信息支持具体回答。
- 文档互相冲突。
- 用户要求承诺、判断或操作，知识库没有授权依据。
- 主模型无法生成有效引用。

客户文案示例：

> 现有资料不足以可靠回答这个问题。我已经把问题和当前会话整理好，交给人工客服继续处理。

同时创建 `knowledge_gap`，保存规范化问题、会话、检索结果和原因。

### “100%”的测试口径

固定验收集中的知识外问题必须 100% 返回拒答/转人工。开放环境不宣传绝对 100%。同时监控知识内误拒率，避免系统为了安全而几乎不回答。
